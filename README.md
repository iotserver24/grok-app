<p align="center">
  <img src="assets/logo.png" alt="Supercharge logo" width="128" height="128" />
</p>
...
This project is derived from the MIT-licensed [`iotserver24/grok-app`](https://github.com/iotserver24/grok-app). See [NOTICE-UPSTREAM.md](NOTICE-UPSTREAM.md) and [LICENSE](LICENSE).
...
Direct SuperGrok billing/account switching, xAI-only media services, Grok Saved album access, and xAI realtime voice are intentionally not part of the Supercharge product.
...
The application uses its own Supercharge data namespace. On first run it can copy compatible non-secret project, session, layout, automation, extension, attachment, skin, and local wallpaper data from the old Grok App location. Existing destination data wins, source data is not deleted, and credentials are never copied automatically.

Legacy `grok://`, `grok-app://`, and `.grokskin` values remain accepted only for migration and import compatibility.
...
MIT. The original Grok App copyright and license notice are preserved in [LICENSE](LICENSE).
